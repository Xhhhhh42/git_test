  ...
  if ( editor_mode_ == MODE_POLYGON ) {
    ...
    if( event.leftDown())
    {
      ...
      if( editor_mode_ == MODE_EDITOR ) {
        map_original_ = map_->map();
        // origin_posi = map_->origin();  
      }
      else if( editor_mode_ == MODE_PRIMITIVE_ELEMENT ) {
        map_original_ = map_ref_->map();
        // origin_posi = map_ref_->origin();  
      } 
      return Render;
    }
...
Q_INVOKABLE void PolygonTool::primitive( std::vector<int> mode, std::vector<double> center, std::vector<int> size )
{
  ...
  if( pri_mode == 0 ) 
  {
    //sphere_test
    int radius = mode[2];
    hector_math::GridMap<float> primitiv( ( 3 + 2 * radius ), ( 3 + 2 * radius ) );
    int rows = primitiv.rows();
    int cols = primitiv.cols();
    // map_primitiv.array().setConstant( std::numeric_limits<float>::quiet_NaN() );
    primitiv.array().setConstant( 0.2 );
    for ( int i = 0; i < rows; i++ ) {
      for ( int j = 0; j < cols; j++ ) {
        if( pow( i - radius - 1, 2 ) + pow( j - radius - 1, 2 ) > pow( radius, 2 ) ) primitiv(i, j) = 0.0f;
      }
    }
    ...
    //cuboid_test
    int height = size[2];
    hector_math::GridMap<float> primitiv( length + 2, width + 2 );
    int rows = primitiv.rows();
    int cols = primitiv.cols();
    ...
    //cube_test
    hector_math::GridMap<float> primitiv( length + 2, length + 2 );
    int rows = primitiv.rows();
    int cols = primitiv.cols();
    ...
    float tanValue = tan( winkel * ( PI / 180.0 ));
...
void PolygonTool::newCenterSubCallback( const geometry_msgs::Point &center )
{
  ...
  if( last_center_.isZero() ) {
    ...
    if( abs( new_pos[2] - last_center_[2] ) > 1E-4 ) {
      ...
      Eigen::Ref<hector_math::GridMap<float>> map_to_editieren = map_->map();
      map_to_editieren = map_original + ( new_pos[2] - last_center_[2] ) / 2; 
      map_ = std::make_shared<Heightmap<float>>(
                map_to_editieren, map_->resolution(), map_->origin(), map_->frame(),
                map_->timestamp() ); 
    }
